...
__global__ void build_hash_table_normal(int* hash_table, int* hash_length, long long* hash_tables_offset, int num_edges, int bucket_num, int* word_id, int* doc_id, int* conflict_count,int bucket_size)
{
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    for (int i = idx; i < num_edges; i += stride)
...
void make_hash_table(float load_factor, int bucket_size)
{
    ...
    cudaMalloc(&d_csr_row, sizeof(int) * (num_nodes + 1));
    cudaMemcpy(d_csr_row, csr_row.data(), sizeof(int) * (num_nodes + 1), cudaMemcpyHostToDevice);
    int* d_max_length;
    cudaMalloc(&d_max_length, sizeof(int));
    calculate_hash_length << <256, 1024 >> > (d_hash_length, num_nodes, d_csr_row, d_max_length, load_factor, bucket_size);
    ...
    build_hash_table_normal << <1024, 1024 >> > (d_hash_table_normal, d_hash_length, d_hash_tables_offset, num_edges, bucket_num, d_vertexs, d_csr_cols, d_conflict_count, bucket_size);
    cudaDeviceSynchronize();
    ...
    cudaMemset(d_conflict_count, 0, sizeof(int));
    start_time = clock();
    build_hash_table_hierarchical << <1024, 1024 >> > (d_hash_table_hierarchical, d_hash_length, d_hash_tables_offset, num_edges, bucket_num, max_length, d_vertexs, d_csr_cols, d_conflict_count,bucket_size);
    cudaDeviceSynchronize();
    end_time = clock();
