...
bool cmp(int a, int b) { return a % max_length < b % max_length; }
...
int main(int argc, char* argv[]) {
  ...
  string inverted_index_offsets_path =
  ...
  cout << "inverted_index_offsets size: " << inverted_index_offsets.size()
       << endl;
  ...
  for (int i = 0; i < inverted_index_num; i++) {
    max_degree = max(max_degree,
                     inverted_index_offsets[i + 1] - inverted_index_offsets[i]);
  }
  printf("max degree: %d\n", max_degree);
  ...
  // for (int i = 0;i<inverted_index_num;i++)
  // {
  //     doc_length[i] = inverted_index_offsets[i + 1] -
  //     inverted_index_offsets[i];
  // }
  ...
  cudaMemcpy(d_query, global_query.data(), global_query.size() * sizeof(int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_query_offsets, query_offsets.data(),
             (query_num + 1) * sizeof(int), cudaMemcpyHostToDevice);

  int* d_inverted_index;
  cudaMalloc(&d_inverted_index, inverted_index.size() * sizeof(int));
  cudaMemcpy(d_inverted_index, inverted_index.data(),
             inverted_index.size() * sizeof(int), cudaMemcpyHostToDevice);
  vector<long long> result_offsets(query_num + 1, 0);
  for (int i = 0; i < query_num; i++) {
    result_offsets[i + 1] =
        result_offsets[i] +
        inverted_index_offsets[global_query[query_offsets[i]] + 1] -
        inverted_index_offsets[global_query[query_offsets[i]]];
  }
  ...
  cudaMemcpy(d_result_offsets, result_offsets.data(),
             (query_num + 1) * sizeof(long long), cudaMemcpyHostToDevice);
  int* d_result;
  ...
  // for (int i = 0; i < inverted_index_num; i++)
  // {
  //     sort(inverted_index.begin() + inverted_index_offsets[i],
  //     inverted_index.begin() + inverted_index_offsets[i + 1], cmp);
  // }
  gpu_sort_csr_cols(inverted_index, inverted_index_offsets, inverted_index_num,
                    max_length);

  printf("finish hierarchical sort\n");
  int* d_inverted_index_hierarchical;
  cudaMalloc(&d_inverted_index_hierarchical,
             inverted_index.size() * sizeof(int));
  cudaMemcpy(d_inverted_index_hierarchical, inverted_index.data(),
             inverted_index.size() * sizeof(int), cudaMemcpyHostToDevice);

  // vector<int> hash_table(20);
  // cudaMemcpy(hash_table.data(), d_hash_table_hierarchical, 20 * sizeof(int),
  // cudaMemcpyDeviceToHost); for (int i = 0; i < 20; i++)
  // {
  ...
  ir_kernel<<<grid_size, block_size>>>(
      ...
      CHUNK_SIZE, max_length, opt, d_hash_table_normal, d_hash_tables_offset,
  ...
  cudaMemcpy(result.data(), d_result, result_offsets.back() * sizeof(int),
             cudaMemcpyDeviceToHost);
  vector<int> result_count(query_num);
  cudaMemcpy(result_count.data(), d_result_count, query_num * sizeof(int),
             cudaMemcpyDeviceToHost);
  int sum1 = 0;
  for (int i = 0; i < query_num; i++) {
    sum1 += result_count[i];
  ...
  time_start = clock();
  ir_kernel<<<grid_size, block_size>>>(
      d_inverted_index_hierarchical, d_inverted_index_offsets, d_query,
      d_query_offsets, query_num, d_result, d_result_offsets, d_result_count,
      d_G_index, CHUNK_SIZE, max_length, true, d_hash_table_hierarchical,
      d_hash_tables_offset, bucket_num, bucket_size);
  cudaDeviceSynchronize();
  time_end = clock();
