            ...
            for (int j = 0, is_not_last = subgraph_degree - 2; j < subgraph_degree; j++)
            {
                ...
                if (level < H - 1 || is_not_last)
                {
                    for (int i = lid; i < candidate_number_previous; i += 32)
                    {
                        ...
                        int is_exist = search_in_hashtable(item, bucket_size, bucket_num, len, cur_hashtable, d_dispersed_mapping);
...
vector<int> createDispersedMapping(int n)
{
...
struct arguments SubgraphMatching(int process_id, int process_num, struct arguments args, char *argv[])
{
    ...
    vector<int> dispersed_mapping = createDispersedMapping(vertex_count);
    int *d_dispersed_mapping;
    HRR(cudaMalloc(&d_dispersed_mapping, vertex_count * sizeof(int)));
    HRR(cudaMemcpy(d_dispersed_mapping, dispersed_mapping.data(), vertex_count * sizeof(int), cudaMemcpyHostToDevice));
    double build_time;
    tie(d_hash_tables_offset, d_hash_tables, bucket_num, build_time) = buildHashTable(d_adjcant, d_vertex, d_degree_offset, d_dispersed_mapping);
    ...
    for (; process_id < process_num; process_id += deviceCount)
    {
        ...
        {
            DFSKernelForClique<<<block_size, block_number>>>(d_reuse, process_id, process_num, chunk_size, length, bucket_size, bucket_num, max_degree, d_intersection_orders, d_intersection_offset, d_restriction, d_degree_offset, d_adjcant, d_hash_tables_offset, d_hash_tables, d_vertex, d_ir, d_sum, G_INDEX, d_dispersed_mapping);
        }
        else
        {
            DFSKernelForGeneral<<<block_size, block_number>>>(process_id, process_num, chunk_size, length, bucket_size, bucket_num, max_degree, d_intersection_orders, d_intersection_offset, d_restriction, d_degree_offset, d_adjcant, d_hash_tables_offset, d_hash_tables, d_vertex, d_ir, d_sum, G_INDEX, d_dispersed_mapping);
        }
        ...
        // HRR(cudaFree(d_ir));
        // HRR(cudaMalloc(&d_ir, (long long)216 * 32 * max_degree * H * sizeof(int)));
        std::cout << "Normal time: " << cmp_time * 1000 << " ms" << std::endl;
        printf("normal preprocessing time: %f ms, kernel time: %f ms\n",
               build_time * 1000.0, cmp_time * 1000.0);
    }
