...
file_path = "../../ir_datasets/ir_figure.txt"
...
try:
    ...
    for line in lines:
        ...
        if len(parts) >= 5:
            dataset = parts[0]
            try:
                # Time Speedup (IR file structure)
                ...
                # 保留两位小数并向上取整
                speedup_ceil = math.ceil(speedup * 100) / 100
                trans_speedup_ceil = math.ceil(trans_speedup * 100) / 100
                
                rows.append({
                    'Dataset': dataset, 
                    'Time Speedup': speedup_ceil,
...
ax.set_title('IR - Speedup', fontsize=16, pad=15, color='#2c3e50', weight='bold')

max_speedup = max(df_shuffled['Time Speedup'].max(), df_shuffled['Trans Speedup'].max())
xmax = np.ceil(max_speedup / 0.25) * 0.25
ax.set_xlim(0.8, xmax)
ax.set_xticks(np.arange(1.0, xmax + 0.01, 0.25))
...
ax.set_title('IR - Coalescing Comparison', fontsize=16, pad=15, color='#2c3e50', weight='bold')
...
plt.savefig('../../../figure/ir_summary_all.pdf', dpi=300, bbox_inches='tight')
print("Generated ir_summary_all.pdf")
plt.close()