...
/**
 * @file sss_roaring.cu
 * @brief SSS任务Roaring位图基线实现 - 扁平化每节点版本
 *
 * 重构特点：
 * - 每个节点独立构建Roaring位图,保存其邻接点
 * - 所有位图扁平化连续存储在一个大数组中
 * - 通过offset数组定位每个节点位图的起始位置
 */

/**
 * @brief SSS扁平化Roaring位图查询内核
 * 每个warp处理一个查询对,动态负载均衡
 * 使用每个节点独立位图查询,O(1)位检测
 */
__global__ void sss_roaring_kernel(RoaringBitmap* bitmap, int num_pairs,
                                   int const* __restrict__ d_vertexs,
                                   int const* __restrict__ d_csr_cols_for_edges,
                                   int const* __restrict__ d_csr_cols,
                                   int const* __restrict__ d_csr_offsets,
                                   int* __restrict__ d_results,
                                   int* __restrict__ d_G_index, int CHUNK_SIZE,
                                   float threshold) {
  ...
  while (pair_idx < num_pairs) {
    ...
    for (int iter = 0; iter < num_iters; iter++) {
      ...
      if (active) {
        ...
        // 在v节点的位图中查找邻接点是否存在
        // O(1)位检测
        found = bitmap->contains(v, neighbor);
      }
...
int find_max_key(const std::vector<int>& csr_cols) {
  int max_key = 0;
  for (int key : csr_cols) {
    if (key > max_key) {
      max_key = key;
    }
  }
...
int run_sss_roaring(int num_pairs, int num_nodes, int const* d_vertexs,
  ...
  new (d_bitmap) RoaringBitmap(num_nodes, csr_offsets_host, csr_cols_host);

  // GPU批量插入所有节点的邻接点
  int failed = d_bitmap->bulk_insert();
  ...
  size_t total_words = d_bitmap->total_words();

  // 清理
  d_bitmap->~RoaringBitmap();
  cudaFree(d_bitmap);
  cudaFree(d_results);
  cudaFree(d_G_index);
