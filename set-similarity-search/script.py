...
def main():
    ...
    command = f"make && ./sss {args.dataset} {extra_args}"
    os.system(command)
    ...
    command = f"TMPDIR=$HOME/tmp /usr/bin/ncu --metrics  {metrics_str} -k set_similarity_search_kernel ./sss {args.dataset} {extra_args}"
    os.system(command)
