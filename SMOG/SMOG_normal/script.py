...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_factor', type=float, default=0.2)
    parser.add_argument(
        '--bucket_size',
        type=int,
        default=5,
    ...
    os.system(command)
    # command = f"TMPDIR=$HOME/tmp /usr/bin/ncu --replay-mode application --metrics l1tex__t_sectors_pipe_lsu_mem_global_op_ld.sum,l1tex__t_requests_pipe_lsu_mem_global_op_ld.sum,smsp__sass_average_data_bytes_per_sector_mem_global_op_ld.pct,l1tex__average_t_sectors_per_request_pipe_lsu_mem_global_op_ld.ratio ./subgraphmatch.bin {input_graph_folder} {input_pattern} {N} {load_factor} {bucket_size} {block_number_for_kernel} {block_size_for_kernel} {chunk_size}"
    # os.system(command)
    ...
    command = f"TMPDIR=$HOME/tmp /usr/bin/ncu --metrics {metrics_str} -k DFSKernelForClique ./subgraphmatch.bin {input_graph_folder} {input_pattern} {N} {load_factor} {bucket_size} {block_number_for_kernel} {block_size_for_kernel} {chunk_size}"
    os.system(command)

    command = f"TMPDIR=$HOME/tmp /usr/bin/ncu --metrics {metrics_str} -k DFSKernelForGeneral ./subgraphmatch.bin {input_graph_folder} {input_pattern} {N} {load_factor} {bucket_size} {block_number_for_kernel} {block_size_for_kernel} {chunk_size}"
    os.system(command)

# clear && export CUDA_VISIBLE_DEVICES=0 &&python script.py --input_graph_folder /data/zh_dataset/processed_graph_challenge_dataset/snap/cit- --input_pattern Q1
