...
void saveUndirectedGraph()
{
    ...
    // vector<int> dispersedMapping = createDispersedMapping(vertex_count);
    // for (int i = 0; i < vertex_count; i++)
    // {
    //     vertex_for_generic[i].vertexID = dispersedMapping[vertex_for_generic[i].vertexID];
    //     for (int j = 0; j < vertex_for_generic[i].edge.size(); j++)
    //     {
    //         vertex_for_generic[i].edge[j] = dispersedMapping[vertex_for_generic[i].edge[j]];
    //     }
    // }
    // sort(vertex_for_generic.begin(), vertex_for_generic.end(), [](const edge_list &a, const edge_list &b)
...
void computeCSR()
{
    ...
    // for (int i = 0; i < vertex_count; i++)
    // {
    //     vertex[i].vertexID = dispersedMapping[vertex[i].vertexID];
    //     for (int j = 0; j < vertex[i].edge.size(); j++)
    //     {
    //         vertex[i].edge[j] = dispersedMapping[vertex[i].edge[j]];
    //     }
    // }
    // sort(vertex.begin(), vertex.end(), [](const edge_list &a, const edge_list &b)
    //      { return a.vertexID < b.vertexID; });
