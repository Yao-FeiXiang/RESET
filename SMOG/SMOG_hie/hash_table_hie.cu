...
tuple<long long *, int *, long long, int> buildHashTable(int *d_adjcant, int *d_vertex, int *d_degree_offset, int *d_dispersed_mapping)
{
    // get bucket_num
    ...
    return make_tuple(d_hash_tables_offset, d_hash_tables_hierarchical, bucket_num, max_length);
}
