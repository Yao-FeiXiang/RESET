...
__global__ void build_hash_table(int* hash_table, int* hash_length,
                                 long long* hash_tables_offset, int num_edges,
                                 int bucket_num, int max_length, int* vertexs,
                                 int* csr_cols, int* conflict_count,
  ...
  for (int i = idx; i < num_edges; i += stride) {
    ...
    int bucket = hierarchical ? d_hash_hierarchical(v, length, max_length)
                              : d_hash_normal(v, length);
    ;
    int k = 0;
...
void make_hash_table(int num_nodes, int** d_hash_length, int** d_csr_offsets,
  ...
  calculate_hash_length<<<256, 1024>>>(*d_hash_length, num_nodes,
                                       *d_csr_offsets, d_max_length,
  ...
  // cudaMemcpy(h_hash_length.data(), *d_hash_length, sizeof(int) * num_nodes,
  ...
  cub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes,
                                *d_hash_length, *d_hash_tables_offset,
                                num_nodes);
  ...
  cudaMemcpy(&last_hash_length, *d_hash_length + num_nodes - 1, sizeof(int),
             cudaMemcpyDeviceToHost);
  long long last_sum;
  cudaMemcpy(&last_sum, *d_hash_tables_offset + num_nodes - 1,
  ...
  cudaMemcpy(d_vertexs, vertexs.data(), sizeof(int) * vertexs.size(),
             cudaMemcpyHostToDevice);
  ...
  build_hash_table<<<1024, 1024>>>(
      *d_hash_table_hierarchical, *d_hash_length, *d_hash_tables_offset,
      csr_cols.size(), bucket_num, max_length, d_vertexs, *d_csr_cols,
      d_conflict_count, bucket_size, hierarchical);
  cudaDeviceSynchronize();
